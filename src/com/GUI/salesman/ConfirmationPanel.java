    ...
    public ConfirmationPanel(){
        ...
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("" +
                    "SELECT o_id ,  m_order.g_id , g_name , m_order.amount , status , submit_time " +
                    "FROM good , m_order " +
                    "WHERE status = '订单已提交'");
            ResultSet resultSet = preparedStatement.executeQuery();
            ResultSetTableModel resultSetTableModel = new ResultSetTableModel(resultSet);
            JTable jTable = new JTable(resultSetTableModel);
            jTable.getColumnModel().getColumn(0).setHeaderValue("订单编号");
            jTable.getColumnModel().getColumn(1).setHeaderValue("商品编号");
            jTable.getColumnModel().getColumn(2).setHeaderValue("商品名称");
            jTable.getColumnModel().getColumn(3).setHeaderValue("订购数量");
            jTable.getColumnModel().getColumn(4).setHeaderValue("订单状态");
            jTable.getColumnModel().getColumn(5).setHeaderValue("提交时间");
